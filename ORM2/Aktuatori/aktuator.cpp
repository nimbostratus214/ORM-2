...
void main(void)
{
    ...
    unsigned int         client_sock; // Multicast socket descriptor
    struct ip_mreq       mreq1;              // Multicast group structure
    struct sockaddr_in   server_addr;       // Client Internet address
    int         server_len;          // Internet address length
    char        poruka1[256];       // Datagram buffer
    int                  retcode1;           // Return code
  ...
  // This stuff initializes winsock
    ...
    server_addr.sin_addr.s_addr = inet_addr("239.255.255.250");
    ...
    /*unsigned char TTL = 1;
    setsockopt(client_sock, IPPROTO_IP, IP_MULTICAST_TTL,
        (char*)&TTL, sizeof(TTL));
        */
    ...
    while (1)
    {
        ...
        if (retcode1 = sendto(client_sock, poruka, sizeof(poruka), 0, (struct sockaddr*)&server_addr, server_len) < 0)
        {
            printf("***NISAM POSLAO");
        }
